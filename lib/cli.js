...
function moveToCommonParent(module, parent, parents, handled) {
    ...
    module.scoped.removeByFn(function (x) {
        ...
        if (topCommonParent) {
            for (var i = 0; i < topCommonParent.outer.arr.length; i++) {
                var child = topCommonParent.outer.arr[i];
                if (child.hasDeep(x)) {
                    topCommonParent.outer.insert(x, i);
                    return true;
                }
            }
            for (var i = 0; i < topCommonParent.scoped.arr.length; i++) {
                var child = topCommonParent.scoped.arr[i];
                if (child.hasDeep(x)) {
...
function moveExportAllImprotsToOuter(module, handled) {
    if (handled === void 0) { handled = new Dictionary_1.Dictionary(); }
    if (handled.has(module)) {
        return;
    }
