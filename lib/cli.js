...
var res_groupByType, res_groupByPage, res_groupByBundle, res_groupByPageAndBundles, res_groupResourcesByType, res_getPage, res_flattern, res_getTreeInfo, res_walk, res_find;
...
(function () {
    (function () {
        exports.res_groupByType = res_groupByType = function (arr, opts) {
        ...
        exports.res_groupByPage = res_groupByPage = function (arr, opts) {
        ...
        exports.res_groupByBundle = res_groupByBundle = function (arr) {
        ...
        exports.res_groupByPageAndBundles = res_groupByPageAndBundles = function (arr, opts) {
        ...
        exports.res_groupResourcesByType = res_groupResourcesByType = function (arr) {
        ...
        exports.res_getPage = res_getPage = function (resource, opts) {
    ...
    (function () {
        exports.res_flattern = res_flattern = function (resource) {
        ...
        function toArray(resource, out) {
            if (resource.resources) {
                resource
                    ...
                    .forEach(function (x) { return toArray(x, out); });
            }
            out['push'](resource);
            return out;
        }
    ...
    (function () {
        ...
        exports.res_getTreeInfo = res_getTreeInfo = function (mix) {
            var arr = Array.isArray(mix) ? mix : flattern(mix);
            var paths = arr.map(function (x) {
                var pages = x.inPages.map(function (page) { return ("bg_white<black<" + page + ">>").color; }).join(' ');
                return x.url + " " + pages;
            }).sort();
            return {
                count: arr.length,
    ...
    exports.res_walk = res_walk = function (res, fn) {
        var result = fn(res);
        if (result === false)
            return result;
    ...
    exports.res_find = res_find = function (res, matcher) {
        var out = null;
...
var SolutionOpts = /** @class */ (function (_super) {
    ...
    function SolutionOpts(solution, opts_) {
        ...
        for (var key in _this.defaults) {
            var val = opts[key] != null ? opts[key] : _this.defaults[key];
            _this[key] = val;
        }
...
exports.HandlersUtils = {
    findPathResolver: function (includeData) {
        var handler = this.find(function (x) { return x.pathResolver && x.pathResolver.accepts && x.pathResolver.accepts(includeData); });
...
var OutputResources = /** @class */ (function () {
    ...
    OutputResources.prototype.getAll = function () {
        var all = this.items.map(function (x) { return x.resource; }).filter(function (x) { return x.embed !== true; });
        if (this.rootOutput) {
...
var BaseBuilder = /** @class */ (function () {
    ...
    BaseBuilder.prototype.accepts = function (type) {
        throw Error('Not implemented');
    };
    return BaseBuilder;
}());
    ...
    MaskRewriter.prototype.rewriteRoot = function (resourceInput, resourceOutput) {
        ...
        global_1.mask.TreeWalker.walk(ast, function (node) {
            if (node.tagName !== 'import')
                return;
            if (path_1.path_isRelative(node.path) === false)
                return;
...
var MaskParser = /** @class */ (function (_super) {
    ...
    MaskParser.prototype._getDependenciesFromNode = function (node) {
        ...
        if (path[0] === '/') {
            var base = global_1.mask.Module.cfg('base');
            if (base) {
                path = path_1.path_combine(base, path);
            }
        }
...
var CommonJsBuilder = /** @class */ (function (_super) {
    ...
    CommonJsBuilder.prototype.wrapWithIIF = function (body) {
        return Templates_1.Templates
            .RootModule
            .replace('%BUNDLE%', function () { return body; });
    };
    CommonJsBuilder.prototype.wrapWithUMD = function (body) {
...
var IncludeJsBuilder = /** @class */ (function (_super) {
    ...
    IncludeJsBuilder.prototype.wrapModule = function (resource, outputItem, otherOutputItems) {
        ...
        var content = resource.content, url = resource.toTargetUrl(this.solution);
    ...
    IncludeJsBuilder.prototype.buildRoot = function (resource, dependencies) {
    };
...
var ScriptParser = /** @class */ (function (_super) {
    ...
    ScriptParser.prototype.getDependencies = function (content, ownerResource) {
        ...
        var dfrs = this.parsers.map(function (parser) { return parser.getDependencies(ast, ownerResource); });
...
var GlobalJsBuilder = /** @class */ (function (_super) {
    ...
    GlobalJsBuilder.prototype.wrapModule = function (resource) {
...
var ScriptBuilder = /** @class */ (function (_super) {
    ...
    ScriptBuilder.prototype.createModule = function (outputItem, otherOutputItems) {
        var _this = this;
        var out = outputItem.resources.map(function (res) {
    ...
    var TreeLoader = /** @class */ (function (_super) {
        ...
        TreeLoader.prototype.process = function () {
            ...
            function toMessage(path) {
                ...
                var name = parts.pop();
                parts = parts.map(function (x) { return ("bold<" + x + ">").color; });
            ...
            reader(this.resource.filename, this.opts).done(function (content) {
                var end = Date.now();
                _this.solution.reporter.print((" cyan<" + (end - start) + "> ms \n").color);
...
var WatcherFactory = /** @class */ (function (_super) {
    ...
    WatcherFactory.prototype.changed = function (filename) {
        ...
        var reporter = this.solution.reporter;
        reporter.info(("File changed bold<cyan<" + path_1.path_getFile(filename) + ">>").color);
        ...
        function build(resource) {
            ...
            return tree_1.tree_async({
                ...
                message: function (treeInfo, seconds) {
                    return ("Created bold<yellow<" + treeInfo.count + ">> files in bold<yellow<" + seconds + ">> sec.").color;
                }
...
(function (Config) {
    ...
    Config.isMultiConfig = isMultiConfig;
    function load() {
        return doLoad([{
                path: 'package.json',
    ...
    function prepair(config) {
        if (isMultiConfig(config)) {
            ...
            var bundles = Object.keys(config.apps).map(function (name) {
                return Object.assign({ name: name }, common_1, config.apps[name]);
            });
            return bundles;
        }
        return [config];
    }
    Config.prepair = prepair;
})(Config || (Config = {}));
var Process;
(function (Process) {
    function processMany(bundles) {
        var i = -1, imax = bundles.length, hasWatcher = bundles.some(function (x) { return x.watch; });
        next();
        function next() {
            if (++i >= imax) {
            ...
            processSingle(bundle).then(function onComplete() {
    ...
    Process.processMany = processMany;
    function processSingle(config) {
        validate(config);
