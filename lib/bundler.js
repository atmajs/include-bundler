			...
			(function () {
				...
				MaskHandler.Parser = class MaskParser extends BaseParser {
					...
					_getDependenciesFromNode (node) {
						...
						if (path.indexOf('box2d') > -1) {
							console.log(node.attr, node.bundle, '<<<<<<<<<<');
						}
						return [ this._createDependency(path, type, page, node.attr.bundle) ];
					}
					...
					_createDependency (path, type, page, bundle) {
						return {
							url: path,
							type: MAPPING[type],
							module: 'mask',
